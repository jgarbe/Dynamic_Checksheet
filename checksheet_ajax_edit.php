...
print("</div>");}
    ...
    $sqlmerg2 ="SELECT ChecksheetName FROM Checksheets WHERE '".checksheetnotochecksheet_id($Checksheetname)."' = Checksheets.merged";
			$resultmerg2 = mysqli_query($dbc, $sqlmerg2) or die ("can't get the Merged2 selection.");  
while ($rowmerg2=mysqli_fetch_array($resultmerg2)) {//Check for merges
	if ($rowmerg2['ChecksheetName']) {// If this Checksheet is part of another Checksheet.
	
	$mergel2 = $rowmerg2['ChecksheetName'];

print("<div style=\"background-color:#214845;text-align:center;\">");
print("".$mergel2." is merged into the Checksheet, ".$Checksheetname.".");
print("</div>"); }
}
...
require_once('inc/get_categories.inc.php');
	$categories=get_categories($Checksheetname);
//	print_r2($categories);
print("<div id=\"accordion\">\n");
foreach($categories as $Ch_NM ){   
		//print("".$Ch_NM."<br />\n");   //Checksheet Name
			foreach ($Ch_NM as $catno) {
				print("<h3><a href=\"#\">".$catno[1]." Category</a></h3>\n<div class=\"yoyo\">\n");
				//	print("".$catno[0]."<br />\n");		//Category ID
				//	print("".$catno[1]."<br />\n");		//Category Name
						print("<table>\n");
					foreach($catno[2] as $subcatno ) {	
				print("<tr><td class=\"th\" colspan=\"3\">".$subcatno[1]." </td></tr>\n");
					//print("".$subcatno[0]."<br />\n");	//Subcategory ID
					//print("".$subcatno[1]."<br />\n");	//Subcategory Name
						foreach($subcatno[2] as $itemarray) {	
							
							print("<tr>\n");
							print("\n");
				print("<td><div class=\"l\">".$itemarray[2]." Subcategory</div></td>\n");
					//print("<td><div class=\"l\">".$itemarray[0]."</div></td>\n");    //thisChecksheet.id
					//print("<td><div class=\"l\">".$itemarray[1]."</div></td>\n");	// Items.id
					//print("<td><div class=\"l\">".$itemarray[2]."</div></td>\n"); //Items.item_name
					print("<td><div class=\"l\">".$itemarray[3]."</div></td>\n");   // How_Many.hm_name
					print("<td><div class=\"l\">".$itemarray[4]."</div></td>\n");		//thisChecksheet.req  required?
								//print_r2($itemarray);
...
?>
<form name="removech" ID="removech" ACTION="<?php $_SERVER['PHP_SELF'] ?>" METHOD="post" >
<?php
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
?>
<form name="editchecksheet" ID="editchecksheet" ACTION="<?php $_SERVER['PHP_SELF'] ?>" METHOD="post" >
<fieldset>
...
?>
<form name="removeitemchecksheets" ID="removeitemchecksheets" ACTION="<?php $_SERVER['PHP_SELF'] ?>" METHOD="post" >
<fieldset>
