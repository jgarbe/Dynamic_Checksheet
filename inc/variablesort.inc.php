...
// If it is a check box type,...
		...
		$variable=1;
	echo "\n\n<!--checkbox-->\n\t<td><div class=chbox>\n";
	echo "\t<Input type=hidden name=".$casc_id."[".$sitem."] value=0>\n";
	echo "\t<INPUT TYPE=CHECKBOX name=".$casc_id."[".$sitem."] value=1 checked />$name</div></td>\n";
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);
	//return array($name,$value,$variable);
} elseif (ctype_digit($value)){  ////////if it's a digit count type,...
	$itemoptions='';
echo "\n\n<!--numerical dropdown-->\n\t<td><div class=l>$name</div>\n";
	for ($n=$value; $n >= 1; $n--) { //dropdown box of
		$l=$n-1;
	    $itemoptions.="<OPTION VALUE=$l>".$l."</OPTION>\n";}
	    echo "<div class=r><SELECT NAME=".$casc_id."[".$sitem."]>\n";
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);
	//return array($name,$value,$variable);
} elseif ($value == 'na'){  ////////if it's an na type,...
		$variable="";
	$itemoptions='';
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
 ...
 elseif ($value == 'open'){ ///////// If it's a two space text input,...
		$variable="";
 	 echo "\n\n<!--open data box-->\n\t<td><div class=l>$name</div><div class=r>\n";
		 echo "<INPUT TYPE=TEXT NAME=".$casc_id."[".$sitem."] SIZE=2></div></td>\n";	 


setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
  }
 elseif ($value == 'O2'){ ////////////If it's an O2 bottle value, ...
		$variable="";
	 echo "\n\n<!--Oxygen Bottle-->\n\t<td><div class=l>$name</div><div class=r>\n";
		 echo "<INPUT TYPE=TEXT NAME=".$casc_id."[".$sitem."] SIZE=4></div></td>\n";	 


setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
}
 elseif ($value == 'irrg'){ /////////// If it's the irrigating solution divided by 500,..
		$variable="";
	echo "\n\n<!--Irrigating Solution-->\n\t<td><div class=l >$name</div>\n";
echo "<div class=r><select NAME=".$casc_id."[".$sitem."]>\n";
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
} elseif ($value == 'tire'){ ////////////////The tire tread option
		$variable="";
	echo "\n\n<!--tire dropdown-->\n\t<td><div class=l >$name</div>\n";
echo "<div class=r><select NAME=".$casc_id."[".$sitem."]>\n";
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
} elseif ($value == 'date'){ ////////////////////////////If it's the date value, ...
	 echo "\n\n<!--date-->\n\t<td><div class=l>$name</div><div class=r>\n";
			$date=date("Y-m-d H:i:s");
		 echo "$date</div></td>\n";
		 echo "<INPUT TYPE='HIDDEN' NAME='".$casc_id."[".$sitem."]' VALUE='$date'></input>";	 }
 elseif ($value == 'miles'){ //////////////////////////If it's an Odometer value, ...
		$variable="";
	 echo "\n\n<!--odometer reading-->\n\t<td><div class=l>$name</div><div class=r>\n";
		 echo "<INPUT TYPE=TEXT NAME=".$casc_id."[".$sitem."] SIZE=7></div></td>\n";
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
} elseif ($value == 'refill'){ /////////If it's a reciprocal table value, ...
	$variable="";
...
echo"<SELECT NAME=".$casc_id."[".$sitem."] >";
?>
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	 //return array($name,$value,$variable);
} elseif ($value == 'personnel'){ ///////////////If it's a reciproca table value, ...
	$variable="";
...
setsql($casc_id,$value,$sitem,$name,$variable,$q_set,$cat,$sub);;
	...
	echo "<div class='comment_' >Notes: (under 200 characters)<TEXTAREA NAME=".$casc_id."[".$sitem."]  ROWS=3 COLS=80></TEXTAREA></div>";
 $dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME); 
	...
	//return array($name,$value,$c_set);
        } else {echo "Bummer, this page is mangled\n";}
//print_r($CHqset);
	}
//******************************************************//
